        ...
        launch {
            try {
                val posts = getPosts(client)
                    .map { post ->
                        async {
                            DataBasketPost(post, getComments(client, post.id))
                        }
                    }.awaitAll()
                    .map {
                        async {
                            DataBasketPost(it.post, it.comments, getAuthor(client, it.getAuthorsId()))
                        }
                    }.awaitAll()
                // Смысла в дальнейшей денормализации не вижу, мапы с ключём по Id автора достаточно
                println(posts)
            } catch (e: Exception) {
                e.printStackTrace()
            }
